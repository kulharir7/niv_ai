...
<body>
    ...
    def create_nbfc_agent(self) -> LlmAgent:
        ...
        tool_names = [
            ...
            "get_document", "search_documents", "create_document", "update_document",
            "nbfc_credit_scoring", "nbfc_loan_prequalification", "nbfc_analytics",
        ]
    ...
    def create_orchestrator(self) -> LlmAgent:
        ...
        return LlmAgent(
            ...
            instruction=(
                ...
                "ROUTING RULES (check in ORDER - first match wins):\n"
                "1. Loan/loans/EMI/NBFC/Borrower/Repayment/disbursed/overdue/DPD/NPA → nbfc_specialist (ALWAYS)\n"
                "2. Code/Script/Field/Create DocType/Customize → frappe_coder\n"
                "3. System scan/DocTypes map/Relationships → system_discovery\n"
                "4. Multi-step complex tasks → niv_planner\n"
                "5. Other data queries → data_analyst\n\n"
                "EXAMPLES:\n"
                "- 'Top 10 loans' → nbfc_specialist (has 'loan')\n"
                "- 'Show loan list' → nbfc_specialist\n"
                "- 'Overdue loans' → nbfc_specialist\n"
                "- 'EMI collection report' → nbfc_specialist\n"
                "- 'Customer CIBIL score' → nbfc_specialist\n"
                "- 'How many customers?' → data_analyst\n"
                "- 'Show sales orders' → data_analyst\n"
                "- 'Create custom field' → frappe_coder\n"
                "- 'Write server script' → frappe_coder\n"
                "- 'What DocTypes exist?' → system_discovery\n\n"
